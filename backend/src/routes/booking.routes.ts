...
/**
 * Confirm payment received (vendor endpoint)
 * POST /api/bookings/:id/confirm-payment-received
 */
router.post('/:id/confirm-payment-received', authenticate, async (req: Request, res: Response) => {
  try {
    const { paymentMethod, paymentDate, amount, currency, transactionReference, notes } = req.body;

    if (!paymentMethod || !paymentDate || !amount || !currency) {
      return res.status(400).json({
        success: false,
        error: {
          code: 'VALIDATION_ERROR',
          message: 'Payment method, date, amount, and currency are required',
          timestamp: new Date().toISOString(),
        },
      });
    }

    const booking = await bookingService.confirmPaymentReceived(
      req.params.id,
      (req as any).user.id,
      {
        paymentMethod,
...
/**
 * Confirm payment sent (organizer endpoint)
 * POST /api/bookings/:id/confirm-payment-sent
 */
router.post('/:id/confirm-payment-sent', authenticate, async (req: Request, res: Response) => {
  try {
    const { paymentMethod, paymentDate, amount, currency, transactionReference, notes } = req.body;
    ...
    const booking = await bookingService.confirmPaymentSent(
      req.params.id,
      (req as any).user.id,
      {
        paymentMethod,
    ...
    res.json({
      success: true,
      data: booking,
      message: 'Payment sent confirmation recorded successfully',
    });
  } catch (error: any) {
    const statusCode = error.message.includes('not found') ? 404 : 
                      error.message.includes('permission') ? 403 : 400;
