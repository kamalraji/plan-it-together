...
export class BookingService {
  ...
  /**
   * Confirm payment received (for direct payment workflow)
   * Only vendors can confirm payment
   */
  async confirmPaymentReceived(
    ...
    // Only vendor can confirm payment
    if (booking.vendor.userId !== vendorUserId) {
      throw new Error('Only the vendor can confirm payment receipt');
    }

    // Check if booking is in correct status
    if (booking.status !== 'QUOTE_ACCEPTED') {
      throw new Error('Payment can only be confirmed for accepted quotes');
    }

    // Update booking status and add payment confirmation
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const updatedBooking = await tx.bookingRequest.update({
        ...
        data: {
          status: 'CONFIRMED',
          additionalNotes: booking.additionalNotes 
            ? `${booking.additionalNotes}\n\nPayment confirmed: ${paymentDetails.paymentMethod} on ${paymentDetails.paymentDate.toLocaleDateString()}${paymentDetails.notes ? ` - ${paymentDetails.notes}` : ''}`
            : `Payment confirmed: ${paymentDetails.paymentMethod} on ${paymentDetails.paymentDate.toLocaleDateString()}${paymentDetails.notes ? ` - ${paymentDetails.notes}` : ''}`,
        },
        include: {
          event: true,
    ...
    console.log(`Payment confirmed for booking ${bookingId} - notifying organizer`);

    return this.mapBookingToResponse(updated);
  }

  /**
   * Request payment confirmation from organizer
   * Organizer can confirm they have sent payment
   */
  async confirmPaymentSent(
    ...
    // Only organizer can confirm payment sent
    if (booking.organizerId !== organizerUserId) {
      throw new Error('Only the organizer can confirm payment sent');
    }
    ...
    // Update booking with payment sent information
    const updated = await prisma.bookingRequest.update({
      where: { id: bookingId },
      data: {
        additionalNotes: booking.additionalNotes 
          ? `${booking.additionalNotes}\n\nPayment sent: ${paymentDetails.paymentMethod} on ${paymentDetails.paymentDate.toLocaleDateString()}${paymentDetails.notes ? ` - ${paymentDetails.notes}` : ''}\nAwaiting vendor confirmation.`
          : `Payment sent: ${paymentDetails.paymentMethod} on ${paymentDetails.paymentDate.toLocaleDateString()}${paymentDetails.notes ? ` - ${paymentDetails.notes}` : ''}\nAwaiting vendor confirmation.`,
      },
      include: {
        event: true,
    ...
    // TODO: Implement notification service call
    console.log(`Payment sent notification for booking ${bookingId} - notifying vendor`);
