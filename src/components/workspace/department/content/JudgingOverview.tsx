...
interface JudgingOverviewProps {
  workspaceId?: string;
}

export function JudgingOverview({ workspaceId }: JudgingOverviewProps) {
  const { data: stats, isLoading, refetch, isFetching } = useJudgingProgress(workspaceId);

  const evaluationProgress = stats && stats.totalAssignments > 0
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-foreground">
                  {stats?.judgedSubmissions || 0}/{stats?.totalSubmissions || 0}
                </span>
              </div>
