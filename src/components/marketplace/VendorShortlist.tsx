...
const VendorShortlist: React.FC<VendorShortlistProps> = ({ eventId, onRequestQuote }) => {
  ...
  // Fetch shortlist items - placeholder since vendor_shortlist table may not exist
  const { data: shortlistItems, isLoading } = useQuery<ShortlistItem[]>({
    queryKey: ['vendor-shortlist', eventId],
    queryFn: async () => {
      // Placeholder: vendor_shortlist table may not exist in current schema
      // Return empty array for now - implement when marketplace tables are created
      return [] as ShortlistItem[];
    },
  });

  // Remove from shortlist mutation - placeholder
  const removeFromShortlistMutation = useMutation({
    mutationFn: async (shortlistItemId: string) => {
      // Placeholder: vendor_shortlist table may not exist
      console.log('Removing from shortlist:', shortlistItemId);
    },
    onSuccess: () => {
  ...
  // Update notes mutation - placeholder
  const updateNotesMutation = useMutation({
    mutationFn: async ({ itemId, notes }: { itemId: string; notes: string }) => {
      // Placeholder: vendor_shortlist table may not exist
      console.log('Updating notes:', itemId, notes);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vendor-shortlist', eventId] });
